...
void fillData(int x, int y, int width, int height, int rows, int cols, const char* data[ROWS][COLS]) {
    int rowHeight = height / rows;
    int colWidth = width / cols;
...
void table(int current_ROWS) {
    ...
    while (current != NULL && row < ROWS) {
        ...
        data[row][2] = current->key;
        data[row][3] = current->dorm;
        current = current->next;
        row++;
    }
    ...
    drawTable(370, 110, 610, 490, ROWS, COLS);
    fillData(370, 110, 610, 490, ROWS, COLS, data);
